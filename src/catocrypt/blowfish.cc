...
bool CBlowfish::Encrypt(unsigned char* buf, size_t n, int iMode)
{
	try {
		//Check the buffer's length - should be > 0 and multiple of 8
		if((n==0)||(n%8!=0)) {
			printf("CBlowfish::Encrypt - Unable to decrypt buffer because it isn't length isn't evenly divisble by 8");
			return false;
		}
		SBlock work;
		...
		printf("Caught exception in CBlowfish::Encrypt");
		return false;
	}
	return true;
}
...
bool CBlowfish::Decrypt(unsigned char* buf, size_t n, int iMode /* = ECB */)
{
	bool ret=false;
	try {
		//Check the buffer's length - should be > 0 and multiple of 8
		if((n==0)||(n%8!=0)) {
			printf("CBlowfish::Decrypt - Unable to decrypt buffer because it isn't length isn't evenly divisble by 8");
			return ret;
		}
		SBlock work;
		if(iMode == CBC) //CBC mode, using the Chain
		...
		printf("Caught exception in CBlowfish::Decrypt");
		return false;
	}
	return true;
}
...
//Returns false if n is multiple of 8
bool CBlowfish::Encrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	try {
		//Check the buffer's length - should be > 0 and multiple of 8
		if((n==0)||(n%8!=0)) {
			printf("CBlowfish::Encrypt - Unable to encrypt buffer because it isn't length isn't evenly divisble by 8");
			return false;
		}
		SBlock work;
		if(iMode == CBC) //CBC mode, using the Chain
...
//Returns false if n is not a multiple of 8
bool CBlowfish::Decrypt(const unsigned char* in, unsigned char* out, size_t n, int iMode)
{
	try {
		//Check the buffer's length - should be > 0 and multiple of 8
		if((n==0)||(n%8!=0)) {
...
char *gensalt() {
    ...
    // This is only for obfuscation purposes...You can't run the strings
    // command over this library and get the salt this way.
    sprintf(salt, "%c%c%c%c%c%c%c%c", 53, 89, 36, 103, 40, 43, 58, 126); 
...
// Decrypt a buffer
long 
decryptBuffer(const char *buffer, long length, char **retBuffer, const char *key) {
	long ret = 0;

    *retBuffer = new char[length];
	if (length > 0) {
        if (*key == '\0')
            key = gensalt();
	CBlowfish blowfish((char *)key);
	if(blowfish.Decrypt((unsigned char *)buffer, (unsigned char *)*retBuffer, length, 0))
		ret = length;
	}
...
// Encrypt a buffer
long 
encryptBuffer(const char *buffer, long length, char **retBuffer, const char *key) {
    if (*key == '\0')
        key = gensalt();
	CBlowfish blowfish((char *)key);
	unsigned char pad = 0;
	...
	long bufferLength = length + pad;

	unsigned char *data = new unsigned char[bufferLength];
	memset(data, pad, bufferLength);
	//memset(data, '\0', bufferLength);
	memcpy(data, buffer, length);

	if (blowfish.Encrypt((unsigned char *)data, bufferLength, 0)) {
		ret = bufferLength;
        *retBuffer = (char *)data;
    }
...
decryptPassword(string ePassword, string &orgPassword, string key) {
    ...
    strncpy(buffer, data, retVal);
    
    unsigned char pad = buffer[retVal - 1];
    buffer[retVal - pad] = 0;
    orgPassword = buffer;
